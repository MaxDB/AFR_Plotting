...
ax = data_dir_execute(@compare_validation,"mems_arch_16","validation error",sol_num,validation_modes);
error_fig = gcf;

Dyn_Data = data_dir_execute(@initalise_dynamic_data,"mems_arch_16");
Dyn_Data = data_dir_execute(@Dyn_Data.validate_solution,sol_num(1),display_mode);
Dyn_Data = data_dir_execute(@Dyn_Data.validate_solution,sol_num(2),display_mode);

phy_amp_backbone_ax = data_dir_execute(@compare_solutions,"physical amplitude","mems_arch_16",sol_num,"validation",1);
